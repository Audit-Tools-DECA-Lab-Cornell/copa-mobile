...
| `copa-mobile/assets/bundled-instrument.json`                 | App-shipped PVUA definition (~394 KB); offline fallback when API + MMKV cache are unavailable.                                                                       |
| `copa-mobile/lib/audit/bundled-instrument.ts`                | Imports bundled JSON, validates with `playspaceInstrumentSchema`, exposes `getBundledInstrument()` and `BUNDLED_INSTRUMENT_VERSION`.                                 |
| `copa-mobile/lib/services/instrument-sync.ts`                | `syncInstrument()` - network fetch, MMKV keys `playspace.instrument_cache` / `playspace.instrument_cache_ts`, then bundled fallback.                                 |
| `copa-mobile/stores/audit-store.ts`                          | `hydrate()` applies MMKV snapshot then eagerly sets bundled instrument when `instrument` is still null; async `syncInstrument()` upgrades store state.               |
| `copa-mobile/lib/audit/types.ts`                             | `playspaceInstrumentSchema`, `persistedAuditStateSchema` (persisted `instrument`), session shapes carrying optional `instrument`.                                    |
| `copa-mobile/lib/i18n/instrument-translations.ts`            | `useLocalizedInstrument()` - returns the audit-scoped or store `instrument` as-is; instrument text has no client-side overlays (backend serves localized copy).      |
| `copa-mobile/lib/audit/api.ts`                               | `createOrResumeAudit` / `fetchAuditSession` - server responses can carry session-scoped `instrument` (does not replace bundled fallback for global store hydration). |
| `copa-mobile/app/execute/[placeId]/index.tsx`                | Execute entry / preamble / mode selection - typical first consumer of `useLocalizedInstrument()`.                                                                    |
| `copa-mobile/app/execute/[placeId]/overview.tsx`             | Section overview - uses instrument for visible sections and progress.                                                                                                |
| `copa-mobile/app/execute/[placeId]/pre-audit.tsx`            | Pre-audit fields driven by `instrument.pre_audit_questions`.                                                                                                         |
| `copa-mobile/app/execute/[placeId]/section/[sectionKey].tsx` | Per-section questions from instrument sections.                                                                                                                      |
| `copa-mobile/app/execute/[placeId]/space-audit.tsx`          | Space-setup step - instrument-driven matrix / pre-audit where applicable.                                                                                            |
| `copa-mobile/MOBILE_OFFLINE_FLOW.mmd`                        | High-level offline persistence / sync diagram including bundled-instrument branch.                                                                                   |
